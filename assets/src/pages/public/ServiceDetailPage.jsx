...
import FeaturePicker from '../../components/forms/FeaturePicker';
        ...
        <Container maxWidth="xl" sx={{ py: 8 }}>
            ...
            <Grid container spacing={6} component={motion.div} {...motionProps}>
                ...
                <Grid item xs={12} md={5}>
                    <Box sx={{
                        position: 'sticky', top: theme.spacing(8),
                        backgroundColor: 'background.paper', borderRadius: 4, p: 4, boxShadow: theme.shadows[4]
                    }}>
                        {loading ? (
                            <Skeleton variant="text" width="60%" height={60} />
                        ) : (
                            <Chip label={service.category} color="primary" sx={{ mb: 3, fontWeight: 600 }} />
                        )}

                        {loading ? (
                            <Skeleton variant="text" width="80%" height={60} sx={{ mb: 3 }} />
                        ) : (
                            <Typography variant="h1" sx={{ fontSize: { xs: '2rem', md: '2.5rem' }, fontWeight: 800, lineHeight: 1.2, mb: 3 }}>
                                {service.name}
                            </Typography>
                        )}

                        {loading ? (
                            <><Skeleton variant="text" /><Skeleton variant="text" /><Skeleton variant="text" width="60%" /></>
                        ) : (
                            <Typography variant="body1" paragraph sx={{ fontSize: '1.1rem', lineHeight: 1.8, mb: 4, color: 'text.secondary' }}>
                                {service.fullDescription}
                            </Typography>
                        )}

                        <Box sx={{ backgroundColor: 'background.default', borderRadius: 3, p: 4, mb: 4 }}>
                                ...
                                {(loading ? Array.from(new Array(4)) : (service?.features || [])).map((feature, index) => (
                                    <Grid item xs={12} sm={6} key={index}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
                                            <CheckCircle sx={{ color: 'primary.main', mr: 2, fontSize: '1.2rem' }} />
                                            {loading ? (
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 4 }}>
                            {loading ? (
                                ...
                                <Box>
                                    ...
                                    <Typography variant="h3" sx={{ color: 'primary.main', fontWeight: 800, fontSize: '2rem' }}>
                                        {service.price}€
                                    </Typography>
                                </Box>
                            )}
                            ...
                            <Button
                                ...
                                sx={{ px: 4, py: 2, fontSize: '1.1rem', fontWeight: 500, whiteSpace: 'nowrap' }}
                                disabled={loading}
                            >
                                {'start project'}
                            </Button>
            ...
            <Modal open={openPicker} onClose={() => setOpenPicker(false)}>
                ...
                >
                    <StartProjectForm service={service} />
                </Box>
            </Modal>
